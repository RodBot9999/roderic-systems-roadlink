...
\newcommand{\fotopendiente}[2]{%
  \begin{figure}[H]
    \centering
    \fcolorbox{RoadTeal}{RoadLight}{%
      \begin{minipage}[c][5.0cm][c]{0.82\textwidth}
        \centering
        {\Large\sffamily\bfseries\color{RoadGreen} Fotografía pendiente}\par\vspace{0.5em}
        {\sffamily #1}\par\vspace{0.45em}
        {\small\color{RoadGray} Sustituir este espacio cuando se proporcione la imagen.}
      \end{minipage}}
    \caption{#2}
  \end{figure}
}
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \vspace{0.8cm}
    {\sffamily\fontsize{17}{22}\selectfont\color{RoadGray}
      Evolución desde las pruebas M2M con SIM800L\par
      hasta la integración del A7670SA en RoadLink}
    \vfill
    ...
    \begin{minipage}{0.82\textwidth}
      ...
      \colorbox{RoadGreen}{\parbox{0.94\textwidth}{\centering\color{white}\sffamily\small
        Documentación técnica del subsistema de comunicación remota}}
    \end{minipage}
    \vspace*{0.8cm}
  \end{center}
\end{titlepage}
...
RoadLink comenzó como una exploración de comunicación remota entre máquinas
(\textit{machine-to-machine}, M2M). La primera meta no fue conectar de inmediato
todos los subsistemas del vehículo, sino comprobar una idea fundamental: que un
microcontrolador pudiera preparar información y enviarla de manera autónoma a
través de una red celular.

Para reducir la cantidad de variables durante las primeras pruebas, se utilizó
...
plataforma celular de la versión actual. El cambio conserva el mismo concepto
general: el ESP32 reúne los datos, controla el módem mediante comandos AT por
UART y entrega una carga útil a un receptor remoto. La implementación se integra
con el resto de RoadLink sin detener la lectura de CAN, GPS, OBD-II, la pantalla
o la interfaz de usuario.

Este documento presenta la evolución, arquitectura, conexiones, protocolo,
pruebas y criterios de integración del subsistema celular. Se omiten revisiones
específicas de las placas, ya que las fotografías del hardware utilizado se
...
local permanente. La solución debe:
...
\begin{itemize}
  ...
  \item transportar datos simulados o datos reales de GPS y OBD-II;
  \item detectar errores, aplicar tiempos límite y reintentar sin bloquear el sistema;
  \item separar la alimentación del módem de la lógica del microcontrolador;
  \item conservar una interfaz de software que permita cambiar el módem sin rediseñar RoadLink completo.
\end{itemize}

El alcance de este informe se concentra en los módulos SIM800L y A7670SA, el
ESP32, la conversión de niveles lógicos y las fuentes reductoras de voltaje. No
se especifica una versión particular de las tarjetas comerciales.
...
La secuencia anterior permitió validar cada capa por separado. Primero se
comprobó el enlace eléctrico y serial; después, el acceso a la red; finalmente,
la integración con los datos reales de RoadLink. Esta estrategia facilita la
localización de fallas porque evita probar al mismo tiempo alimentación, UART,
registro celular, adquisición vehicular y servidor.

\section{Arquitectura del subsistema}

La Figura~\ref{fig:arquitectura-celular} resume las conexiones funcionales. La
entrada del sistema alimenta dos convertidores reductores. Una salida se reserva
para el módem celular y la otra alimenta la electrónica lógica. El enlace UART
entre el ESP32 y el módem atraviesa un conversor bidireccional de nivel.
...
\begin{itemize}
  ...
  \item adaptación de niveles entre el controlador y el módem;
  \item separación entre adquisición de datos y transporte celular.
\end{itemize}

\subsection{Funciones de cada bloque}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{4.1cm} Y}
    ...
    SIM800L & Plataforma de las primeras pruebas de SMS y comunicación de datos. \\
    A7670SA & Plataforma celular utilizada en la integración actual de RoadLink. \\
    \shortstack[l]{Conversor\\bidireccional} & Adapta los niveles lógicos del enlace UART cuando es necesario. \\
...
La prueba utiliza el modo de texto del módem. El fragmento siguiente muestra la
secuencia mínima y utiliza nombres simbólicos para los pines y el número de
destino.

\begin{lstlisting}[style=roadcode,caption={Secuencia mínima para una prueba de SMS.}]
...
detectar errores y limitar los tiempos de espera. El fragmento se conserva
pequeño porque documenta la idea de la prueba, no sustituye al servicio celular.
...
\fotopendiente{ESP32, fuente de alimentación y SIM800L utilizados en la prueba inicial.}{Montaje de la primera prueba M2M con SIM800L.}

\section{Integración actual con A7670SA}

En la etapa actual, el A7670SA cumple la función de transporte celular de
RoadLink. El cambio de módem no modifica el origen de la telemetría ni el
formato recibido por el servidor. El ESP32 continúa entregando comandos y datos
por UART; el módem establece la sesión de red y envía la solicitud HTTP.
...
Mientras el módem espera registro, conexión o respuesta HTTP, RoadLink continúa
actualizando CAN, GPS, OBD-II, la interfaz web y la pantalla.
...
El nombre histórico de la clase del repositorio corresponde al desarrollo con
SIM800L. Para la integración con A7670SA se conserva la interfaz del servicio y
se adapta internamente la secuencia de comandos requerida por el módem. Así,
GPS, OBD-II, el menú y el receptor no necesitan conocer el modelo celular.
...
Las instrucciones AT concretas pueden variar entre familias de módem. Por ello,
la arquitectura define el resultado esperado ---abrir datos y ejecutar un POST---
sin afirmar que todos los módulos implementan exactamente los mismos comandos.
...
en memoria no volátil. El receptor valida la clave y registra la telemetría.

\fotopendiente{ESP32, alimentación, conversor de nivel y A7670SA utilizados en la integración actual.}{Montaje de integración del A7670SA con RoadLink.}

\section{Alimentación y niveles eléctricos}

Un módem celular puede producir cambios rápidos de corriente al registrarse o
transmitir. Una fuente estable en reposo puede sufrir una caída de voltaje
...
referencia común a UART. El conversor bidireccional evita aplicar directamente
un nivel lógico incompatible entre el ESP32 y el módem.
...
\begin{table}[H]
  \centering
  \caption{Plan resumido de pruebas.}
  \begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{3.4cm} Y Y}
    \toprule
    Prueba & Resultado esperado & Evidencia recomendada \\
    \midrule
    Comunicación UART & Respuesta \texttt{OK} a \texttt{AT} & Captura del monitor serial. \\
    SMS & Recepción del texto generado & Fotografía del mensaje recibido. \\
    \shortstack[l]{Registro\\celular} & SIM lista, red registrada y señal válida & Registro de comandos. \\
    Datos HTTP & Respuesta exitosa del receptor & Registro del servidor. \\
    \shortstack[l]{Carga\\eléctrica} & Sin reinicios durante transmisión & Medición de voltaje. \\
    Recuperación & Retorno después de pérdida de red & Registro del reintento. \\
    \bottomrule
  \end{tabularx}
\end{table}

En cada prueba se deben anotar fecha, firmware, configuración, carga enviada,
resultado real y observaciones. Números telefónicos, identificadores de SIM,
claves y direcciones privadas deben ocultarse antes de publicar evidencias.
...
Las siguientes mejoras documentales serán incorporar fotografías reales,
resultados medidos y capturas de las pruebas. Estas evidencias permitirán
relacionar cada decisión de diseño con el prototipo físico.

\section*{Fuentes internas del proyecto}
\addcontentsline{toc}{section}{Fuentes internas del proyecto}

\begin{itemize}
  \item \texttt{firmware/RoadLink/RoadLink.ino}: integración de servicios en el ciclo principal.
  \item \texttt{firmware/RoadLink/Sim800Service.cpp}: estados, AT, JSON y HTTP POST en la rama celular.
  \item \texttt{firmware/RoadLink/AppConfig.h}: parámetros de UART y tiempos límite.
  \item \texttt{docs/SIM800L\_TELEMETRY.md}: cableado y procedimiento de prueba.
\end{itemize}
